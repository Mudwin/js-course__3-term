...
function checkNetworkStatus() {
  const interval = setInterval(async () => {
    ...
    try {
      ...
      if (duration > 500) {
        const timeError = new Error("Медленное соединение");
        timeError.name = "SlowConnectionError";
        throw timeError;
      }

      renderNetworkStatus(null);
    } catch (error) {
      if (error.name === "SlowConnectionError") {
        renderNetworkStatus(error.message);
      } else {
        renderNetworkStatus("Неполадки с сетью");
      }
    }
  }, 5000);
}
...
function renderNetworkStatus(message) {
  ...
  if (!notification) {
    ...
    if (message == "Медленное соединение") {
      notification.id = "time-error";
    } else {
      notification.id = "network-error";
    }
  }
